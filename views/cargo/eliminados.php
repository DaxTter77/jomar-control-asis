...
<script type="text/javascript">
    function goToPage(){
    self.location.replace('/Jomar/users_control/controller/CargoController.php?action=eliminados');   
    }
    ...
    function consultarDatos(){
        ...
        $.ajax({
            data: datos,
            type: 'post',
            url: "/Jomar/users_control/controller/CargoController.php?action=eliminados",
            dataType: 'json',
            success: function(response){
                    if(response != null){                        
                        ...
                        $.each(response, function(i){                        
                            ...
                            + "<td>"+ this.cargo+"</td>"
                            + "<td> <a href='javascript: restaurarCargo(\""+this.id_cargo+"\")' class='badge badge-warning'>Restaurar </a> </td></tr>");
                        });
                    }else{
                        $("#contentTable").html("");
    ...
    function restaurarCargo(id){
        //floatDestroy();
        restaurar = confirm("¿Desea restaurar este cargo?");
        if(restaurar){
            $.ajax({                        
                url: "/Jomar/users_control/controller/CargoController.php?action=restaurarCargo&id_cargo="+id,
                success: function(response){ 
                    if(response.trim() == "1"){
                        consultarDatos();
...
<body style="background-color:#800D0D0D">
    ...
    <div class='container text-center shadow p-5 mb-5 mt-3 bg-white rounded'>
        <h1> Cargos eliminados </h1>    
    </div>
    <div class='alert-success container w-75 text-center mb-5' id="mensaje"></div>
    <?php
        $url = $_SERVER["REQUEST_URI"];
        $urlneed = "/Jomar/users_control/controller/CargoController.php?action=eliminados";
        if($url != $urlneed){
    ...
    <div class='container text-center shadow p-5 mb-5 bg-white rounded'>    
        ...
        <?php        
            if($datos == null){
                echo "<br> <h3> No hay cargos eliminados <h3>";
        ...
        ?>
            <div class='d-flex justify-content-center' id="contentTable">
                <table class='table table-stripped' border=1 id="table">
                    ...
                    <tbody>
                        <a href="/Jomar/users_control/controller/CargoController.php?action=eliminados"></a>
                        <?php
                            for($i = 0; $i < count($datos); $i++){
                                $id = $datos[$i]['id_cargo'];                            
                                $rutaEl = "/Jomar/users_control/controller/CargoController.php?action=restaurarCargo&id_cargo=$id";
                                echo "<tr>";
                                echo "<td>". ($i+1) ."</td>";
                                echo "<td> {$datos[$i]['cargo']} </td>";                           
                                echo "<td>                            
                                <a href='javascript: restaurarCargo(\"$id\")' class='badge badge-warning'>Restaurar</a>
                                </td>";
                                echo "</tr>";
                            }
        ...
        <a class="btn btn-info" href="/Jomar/users_control/controller/CargoController.php?action=todos">Volver</a>        
        </div> 
    </div>
</body>
</html>