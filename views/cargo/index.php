...
<head>
    <meta charset="UTF-8">
    <link rel="icon" href="/Jomar/users_control/images/favicon.png" type="image/png">
...
<script type="text/javascript">
    function goToPage(){
    self.location.replace('/Jomar/users_control/controller/CargoController.php?action=todos');
    }
    ...
    function consultarDatos(){
        ...
        $.ajax({
            data: datos,
            type: 'post',
            url: "/Jomar/users_control/controller/CargoController.php?action=todos",
            dataType: 'json',
            success: function(response){
                    if(response != null){
                        var rutaEd = "/Jomar/users_control/controller/CargoController.php?action=editar&id_cargo="+this.id_cargo;
                        $("table tbody").html("");
                        $.each(response, function(i){                        
                            $("table tbody").append("<tr><td>"+ (i+1) +"</td>"                            
                            + "<td>"+ this.cargo+"</td>"
                            + "<td> <a href='"+rutaEd+"' class='badge badge-primary'>Editar </a> <a href='javascript: eliminarEmp(\""+this.id_cargo+"\")' class='badge badge-warning'>Eliminar </a> </td></tr>");
                        });
                    }else{
                        $("#contentTable").html("");
    ...
    function eliminarCargo(id){
        //floatDestroy();
        eliminar = confirm("¿Desea eliminar este cargo?");
        if(eliminar){
            $.ajax({                        
                url: "/Jomar/users_control/controller/CargoController.php?action=eliminar&id_cargo="+id,
                success: function(response){                
                    if(response.trim() == "1"){
                        consultarDatos();
...
<body style="background-color:#800D0D0D">
    ...
    <?php
        ...
        $urlneed = "/Jomar/users_control/controller/CargoController.php?action=todos";
        if($url != $urlneed){
            echo "<script> goToPage() </script>";
        }
    ?>
    <div class='container text-center shadow p-5 mb-5 bg-white rounded'>
        <a class='btn btn-success btn-lg btn-block' href="/Jomar/users_control/controller/CargoController.php?action=insertar">Ingresar</a>
        ...
        <?php        
            if($datos == null){
                echo "<br> <h3> No hay cargos en el sistema <h3>";
        ...
        ?>
            <div class='d-flex justify-content-center' id="contentTable">
                <table class='table table-stripped' border=1 id="table">
                    ...
                    <tbody>
                        <a href="/Jomar/users_control/controller/CargoController.php?action=todos"></a>
                        <?php
                            for($i = 0; $i < count($datos); $i++){
                                $id = $datos[$i]['id_cargo'];
                                $rutaEd = "/Jomar/users_control/controller/CargoController.php?action=editar&id_cargo=$id";
                                $rutaEl = "/Jomar/users_control/controller/CargoController.php?action=eliminar&id_cargo=$id";
                                echo "<tr>";
                                echo "<td>". ($i+1) ."</td>";
                                echo "<td> {$datos[$i]['cargo']} </td>";                                                         
                                echo "<td>
                                <a href='$rutaEd' class='badge badge-primary'>Editar </a>
                                <a href='javascript: eliminarCargo(\"$id\")' class='badge badge-warning'>Eliminar </a>
                                </td>";
                                echo "</tr>";
                            }
        ...
        <div class="row">
            ...
            <div class="justify-content-end row">
                <a class="btn btn-info" href="/Jomar/users_control/controller/CargoController.php?action=eliminados">Cargos Eliminados</a>
            </div>
        </div>
    </div>
</body>
