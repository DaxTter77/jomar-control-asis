...
<script type="text/javascript">
    function goToPage(){
    self.location.replace('/Jomar/users_control/controller/SedeController.php?action=todos');
    }
    ...
    function consultarDatos(){
        ...
        $.ajax({
            data: datos,
            type: 'post',
            url: "/Jomar/users_control/controller/SedeController.php?action=todos",
            dataType: 'json',
            success: function(response){
                    if(response != null){
                        var rutaEd = "/Jomar/users_control/controller/SedeController.php?action=editar&id_sede="+this.id_sede;
                        $("table tbody").html("");
                        $.each(response, function(i){                        
                            $("table tbody").append("<tr><td>"+ (i+1) +"</td>"
                            + "<td>"+ this.sede+"</td>"
                            + "<td> <a href='"+rutaEd+"' class='badge badge-primary'>Editar </a> <a href='javascript: eliminarSede(\""+this.id_sede+"\")' class='badge badge-warning'>Eliminar </a> </td></tr>");
                        });
                    }else{
                        $("#contentTable").html("");
    ...
    function eliminarSede(id){
        //floatDestroy();
        eliminar = confirm("¿Desea eliminar esta sede?");
        if(eliminar){
            $.ajax({                        
                url: "/Jomar/users_control/controller/SedeController.php?action=eliminar&id_sede="+id,
                success: function(response){                
                    if(response.trim() == "1"){
                        consultarDatos();
...
<body style="background-color:#800D0D0D">
    ...
    <?php
        ...
        $urlneed = "/Jomar/users_control/controller/SedeController.php?action=todos";
        if($url != $urlneed){
            echo "<script> goToPage() </script>";
        }
    ?>    
    <div class='container text-center shadow p-5 mb-5 bg-white rounded'>
        <a class='btn btn-success btn-lg btn-block' href="/Jomar/users_control/controller/SedeController.php?action=insertar">Ingresar</a>
        ...
        <?php        
            if($datos == null){
                echo "<br> <h3> No hay sedes en el sistema <h3>";
        ...
        ?>
            <div class='d-flex justify-content-center' id="contentTable">
                <table class='table table-stripped' border=1 id="table">
                    ...
                    <tbody>
                        <a href="/Jomar/users_control/controller/SedeController.php?action=todos"></a>
                        <?php
                            for($i = 0; $i < count($datos); $i++){
                                $id = $datos[$i]['id_sede'];
                                $rutaEd = "/Jomar/users_control/controller/SedeController.php?action=editar&id_sede=$id";
                                $rutaEl = "/Jomar/users_control/controller/SedeController.php?action=eliminar&id_sede=$id";
                                echo "<tr>";
                                echo "<td>". ($i+1) ."</td>";
                                echo "<td> {$datos[$i]['sede']} </td>";                                                         
                                echo "<td>
                                <a href='$rutaEd' class='badge badge-primary'>Editar </a>
                                <a href='javascript: eliminarSede(\"$id\")' class='badge badge-warning'>Eliminar </a>
                                </td>";
                                echo "</tr>";
                            }                        
                        ?>
        ...
        <div class="row">
            ...
            <div class="justify-content-end row">
                <a class="btn btn-info" href="/Jomar/users_control/controller/SedeController.php?action=eliminados">Sedes Eliminadas</a>
            </div>
        </div>
    </div>
</body>
