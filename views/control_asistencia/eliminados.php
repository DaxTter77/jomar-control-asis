...
<script type="text/javascript">
    ...
    function consultarDatos(){
        var url = "/Jomar/users_control/controller/ControlAsisController.php?action=eliminados"; 
        var datos = {
            'confirm' : true
        };
        $.ajax({
            ...
            success: function(response){                                
                    if(response != null){                        
                        ...
                        $.each(response, function(i){                        
                            ...
                            + "<td> </a> <a href='javascript: restaurarRegistro(\""+this.id_controlAsis+"\")' class='badge badge-warning'>Restaurar </a> </td></tr>");
                        });                    
                    }else{
                        $("#contentTable").html("");
    ...
    function restaurarRegistro(id){
        //floatDestroy();
        restaurar = confirm("¿Desea restaurar este registro?");
        if(restaurar){
            $.ajax({                        
                url: "/Jomar/users_control/controller/ControlAsisController.php?action=restaurarRegistro&id_controlAsis="+id,
                success: function(response){
                    if(response.trim() == "1"){                    
                        consultarDatos();
                        ...
                        $("#mensaje").html("<h4>No se ha podido restaurar</h4>");
...
<body style="background-color:#800D0D0D">
    ...
    <div class='container text-center shadow p-5 mb-5 bg-white rounded'>    
        ...
        <?php        
            if($datos == null){
                echo "<br> <h3> No hay registros eliminados <h3>";
        ...
        ?>        
            <div class='d-flex justify-content-center' id="contentTable">
                <table class='table table-stripped' border=1 id="table">
                    ...
                    <tbody>
                        <a href="/Jomar/users_control/controller/ControlAsisController.php?action=eliminados"></a>
                        <?php
                            for($i = 0; $i < count($datos); $i++){
                                ...
                                echo "<td style='text-align: justify; width: 200px;'> {$datos[$i]['nota']} </td>";                            
                                echo "<td>                       
                                <a href='javascript: restaurarRegistro(\"$id\")' class='badge badge-warning'>Restaurar</a>
                                </td>";
                                echo "</tr>";
                            }
        ...
        <a class="btn btn-info" href="javascript: volver()">Volver</a>
        </div> 
    </div>
</body>
</html>